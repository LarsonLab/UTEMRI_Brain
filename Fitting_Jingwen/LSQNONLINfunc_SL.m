function [fitparam, time_NONLIN, rmse, S_fit] = ...
    LSQNONLINfunc_SL(S0all, Acq, multipeak, initCond, lb, ub, tRF, iterMax, Wall)
...
if nargin < 5
        % PDwr PDmr PDwi PDmi R2w  T2m   R1w R1m  DFw
    lb = [-inf -inf -inf -inf 0.5  5     0.2 0.2  -inf];
    ub = [+inf +inf +inf +inf 100  1000  10  10   +inf];
end

if nargin < 4
              % PDwr PDmr PDwi PDmi R2w T2m  R1w R1m DFw
    initCond = [10   1    0    0    25  10   0.5 2   30];
end
...
%% cost function
    function [res, S] = cost_comp1(param)
        ...
        % SL decay function
        time_fun = @(TE,theta,t2) ...
            sin(theta).*exp(-TE/t2*abs(3*cos(theta).^2-1)); % /(2*pi);
        ...
        for nn = 1:length(Acq.TE(:))
            ...
            % SL decay function
            SLdecay_TE = integral(@(theta) time_fun(Acq.TE(nn),theta,T2m/1e6),0,pi/2);
