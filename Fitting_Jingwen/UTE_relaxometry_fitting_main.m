...
% FA2: 12-deg
load([Pfile_12deg suffix]);

imall_all{2} = imall .* (10 ^ (3 * (header_18.PrescanHeader.aps_r1 - header_12.PrescanHeader.aps_r1) / 20)) ...
    .* (2 ^ (header_18.PrescanHeader.aps_r2 - header_12.PrescanHeader.aps_r2));% * 1e4;
imallplus_all{2} = imallplus .* (10 ^ (3 * (header_18.PrescanHeader.aps_r1 - header_12.PrescanHeader.aps_r1) / 20)) ...
    .* (2 ^ (header_18.PrescanHeader.aps_r2 - header_12.PrescanHeader.aps_r2));% * 1e4;
flips(2) = 12*pi/180;
TEin{2} = TE;

% FA3: 6-deg
load([Pfile_06deg suffix]);

imall_all{1} = imall .* 10 ^(3 * (header_18.PrescanHeader.aps_r1 - header_06.PrescanHeader.aps_r1) /20) ...
    .* (2 ^ (header_18.PrescanHeader.aps_r2 - header_06.PrescanHeader.aps_r2));% * 1e4;
imallplus_all{1} = imallplus .* 10 ^(3 * (header_18.PrescanHeader.aps_r1 - header_06.PrescanHeader.aps_r1) /20) ...
    .* (2 ^ (header_18.PrescanHeader.aps_r2 - header_06.PrescanHeader.aps_r2));% * 1e4;
flips(1) = 6*pi/180;
TEin{1} = TE;
...
for ii = 1:3
    ...
    Inputs.Phase = angle(im_masked{ii}(:,:,:,end));
    Inputs.Mask = double(brainmask_cut);
    Unwrapped = SEGUE(Inputs);
    im_masked_unwrap{ii}(:,:,:,end) = abs(im_masked{ii}(:,:,:,end)).*exp(1i*Unwrapped);
end
...
imagesc(imrotate(squeeze(angle(flip(im_masked{1}(50,:,:,end)))), 270));
subplot(234)
imagesc(squeeze(angle(flip(im_masked_unwrap{1}(:,:,46,end))))); colormap gray
subplot(235)
imagesc(imrotate(squeeze(angle(flip(im_masked_unwrap{1}(:,50,:,end)))), 270));
subplot(236)
imagesc(imrotate(squeeze(angle(flip(im_masked_unwrap{1}(50,:,:,end)))), 270));
...
general_opts.methylene_freq_est = 2.31*B0*42.57e-3; % kHz % 3.5
...
%     [fitting_result(Ix)] = ...
%         UTE_joint_fitting_function(TEin, Sin, aflips, TR, B0, phi_RF, plot_flag);
...
figure('position',[100 100 1600 600]); 
ax(1) = subplot(241);
imagesc(flip(fit_maps{1}.rho),[0 7]); colorbar; title('Comp1 rho');
ax(2) = subplot(242); 
imagesc(flip(fit_maps{2}.rho),[0 0.7]); colorbar; title('Comp2 rho');
ax(3) = subplot(243); 
imagesc(flip(fit_maps{2}.rho_frac),[0 0.5]); colorbar; title('Comp2 fraction');
ax(4) = subplot(244); 
imagesc(flip(fit_maps{1}.AIC)); colorbar; title('AIC');
ax(5) = subplot(245); 
imagesc(flip(fit_maps{1}.T1),[0 5]); colorbar; title('Comp1 T1');
ax(6) = subplot(246); 
imagesc(flip(fit_maps{2}.T1),[0 0.5]); colorbar; title('Comp2 T1');
ax(7) = subplot(247); 
...
histogram(nonzeros(fit_maps{2}.df/B0/42.57e-3),-2:0.1:6); hold on;
