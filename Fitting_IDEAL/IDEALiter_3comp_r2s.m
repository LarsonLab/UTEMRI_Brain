function [Sdemod, Ssubt, S_fit, phi_final, R2s_final, rho_final, phi_delta, R2s_delta] = ...
    IDEALiter_3comp_r2s(S0, TE, phi_iter, R2s_iter, chemshift)
...
C(:,2) = cos(2*pi*0*TE).*exp(-abs(R2s_iter(2))*TE);
D(:,2) = sin(2*pi*0*TE).*exp(-abs(R2s_iter(2))*TE);
C(:,3) = cos(2*pi*chemshift*TE).*exp(-abs(R2s_iter(3))*TE);
D(:,3) = sin(2*pi*chemshift*TE).*exp(-abs(R2s_iter(3))*TE);
...
% forcing phase to be zero or small ??
% rhoC = rhoR + 1i*rhoI;
% rhoR = abs(rhoC);
% rhoI = angle(rhoC);
% rhoI(rhoR < 0) = -rhoI(rhoR < 0);
% rhoR(rhoR < 0) = -rhoR(rhoR < 0);
...
C(:,2) = cos(2*pi*0*TE).*exp(-R2s_final(2)*TE);
D(:,2) = sin(2*pi*0*TE).*exp(-R2s_final(2)*TE);
C(:,3) = cos(2*pi*chemshift*TE).*exp(-R2s_final(3)*TE);
D(:,3) = sin(2*pi*chemshift*TE).*exp(-R2s_final(3)*TE);
