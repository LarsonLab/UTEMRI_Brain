function [Sdemod, Ssubt, S_fit, phi_final, R2s_final, R1_final, rho_final, phi_delta, R2s_delta, R1_delta] = ...
    IDEALiter_2comp_r2sr1_v3(S0, TE, TR, FA, phi_iter, R2s_iter, R1_iter, multipeak)
...
Chat(:,1) = cos(2*pi*0*TE).*exp(-abs(R2s_final(1))*TE).*sin(FA);
Dhat(:,1) = sin(2*pi*0*TE).*exp(-abs(R2s_final(1))*TE).*sin(FA);
Chat(:,2) = (cos(2*pi*TE*multipeak.chemshift)*multipeak.alpha').*exp(-abs(R2s_final(2))*TE).*sin(FA);
Dhat(:,2) = (sin(2*pi*TE*multipeak.chemshift)*multipeak.alpha').*exp(-abs(R2s_final(2))*TE).*sin(FA);
...
for nn = 1:Ncomp
    E1 = exp(-TR*abs(R1_iter(nn)));
    alpha = taylorX(FA, E1, TR);
    F(:,nn) = repmat(alpha,[2 1])...
        .*([Chat(:,nn), -Dhat(:,nn); Dhat(:,nn), Chat(:,nn)]*[rhoR_final(nn);rhoI_final(nn)]);
end
...
function alpha = taylorX(FA, E1, TR)

alpha = E1*TR./(1-cos(FA)*E1);
for i = 1:1000
    alpha = alpha - i*(cos(FA)*E1).^i*(1-E1)*TR;
end
