...
[coeff,score,latent,tsquared,explained,mu] = pca(Sinput_orig);
keepInd = find(cumsum(explained) > 99, 1);
score(:,keepInd+1:end) = 0;
Sinput_pca = score*coeff' + repmat(mu,size(score,1),1);

%% denoising - Low rank svd truncation

Sinput_origR = real(Sinput_orig);
Sinput_origI = imag(Sinput_orig);
Sig = [Sinput_origR Sinput_origI];
...
Sinput_svd = Sig_svd(:,1:24) + 1i*Sig_svd(:,25:end);
...
export_fig(sprintf('IDEAL_slice_complex'),'-png','-transparent'); close;
