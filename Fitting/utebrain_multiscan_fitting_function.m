function [fit_result1, AIC1, fit_result2, AIC2, fit_result2m, AIC2m, fit_result3, AIC3] = ...
    utebrain_multiscan_fitting_function(TEin, Sin, B0, phi_est, plot_flag)
...
[fit_result1, rmse1, AIC1, TEfit, Sfit] = utebrain_model_fit(TEin,Sin,fit_params, general_opts);
general_opts.plot_flag = plot_flag;

% remove long-T2 phase and frequency (easier to see...)
Sin_corrected = Sin(:) .* exp(i* (2*pi*fit_result1(1).df .* TEin(:) - fit_result1(1).phi) );
...
if 1
    ...
    if 1
        ...
        general_opts.plot_flag = 0;
        [fit_result2m, rmse2m, AIC2m, TEfit, Sfit] = utebrain_model_fit(TEin(IuT2),Sin_corrected(IuT2),fit_params, general_opts);
    ...
    general_opts.plot_flag = 0;
    [fit_result2, rmse2, AIC2, TEfit, Sfit] = utebrain_model_fit(TEin(IuT2),Sin_corrected(IuT2),fit_params, general_opts);
    
    % remove long-T2 phase and frequency again
    Sin_corrected = Sin_corrected(:) .* exp(i* (2*pi*fit_result2(1).df .* TEin(:) - fit_result2(1).phi) );
    
    general_opts.plot_flag = plot_flag;
    if plot_flag
        [fit_result1, rmse1, AIC1, TEfit, Sfit] = utebrain_model_fit(TEin,Sin_corrected,fit_params1, general_opts1);
    end
