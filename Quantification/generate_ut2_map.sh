...
# Usage: ./generate_ut2_map.sh -m <MPRAGE_dicom_folder> -te1 <te1.nii.gz> -te2 <te2.nii.gz> -o <output_directory>
...
echo "Computing TE2 normalization factor..."

MPRAGE_SEG="${OUTDIR}/fast_mprage_seg.nii.gz"
GM_MASK="${WORKDIR}/mprage_gm_mask.nii.gz"

# Get mask label 2 (csf) from FAST segmentation
fslmaths "${MPRAGE_SEG}" -thr 2 -uthr 2 -bin "${GM_MASK}"

# Compute mean signal in csf for both TE1 and TE2 registered images
...
if (( $(echo "${MEAN_TE2} == 0" | bc -l) )); then
  echo "Error: Mean TE2 signal is zero, cannot normalize" >&2
  exit 1
fi

TE2_NORMALIZATION_FACTOR=$(echo "${MEAN_TE1} / ${MEAN_TE2}" | bc -l)
echo "Computed TE2 normalization factor: ${TE2_NORMALIZATION_FACTOR}"
