...
%       UTERosette_bSSFP_dual_echo_reconstruction_Siemens(cfg);
    ...
    img_recon_te1 = flip(image_recon_te1, 1);
    ...
    img_recon_te2 = flip(image_recon_te2, 1);
    
    % -- Save outputs --
    % Create subdirectory for results
    out_subdir = fullfile(config.io.out_path, 'UTERosette_bSSFP_dual_echo');
    if ~exist(out_subdir, 'dir')
        mkdir(out_subdir);
    end

    % Save config, recon images, and trajectories
    save(fullfile(out_subdir, 'config.mat'), 'config');
    save(fullfile(out_subdir, 'img_recon.mat'), 'img_recon_te1', 'img_recon_te2');
    save(fullfile(out_subdir, 'trajectory.mat'), 'trajectory_te1', 'trajectory_te2');
    niftiwrite(img_recon_te1, fullfile(out_subdir, 'img_recon_te1.nii'));
    niftiwrite(img_recon_te2, fullfile(out_subdir, 'img_recon_te2.nii'));
